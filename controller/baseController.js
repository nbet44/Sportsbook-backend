...
exports.Bupdate = async (model, condition = {}, set = {}) => {
	try {
		var findhandle = null;
		await model.updateMany(condition, set).then(rdata => {
			findhandle = rdata;
		});
		if (!findhandle) {
			return false;
...
exports.BfindSort = async (model, condition = {}, sortType = { order: 1 }) => {
	try {
		var outdata = null;
		await model.find(condition).sort(sortType).then(rdata => {
			if (!rdata) {
				outdata = false;
			} else {
				outdata = rdata;
...
exports.imageupload = (req, res, next) => {
	if (req.files && req.files.length) {
		...
		for (var i in req.files) {
			...
			var now_path = path.resolve("./uploads/") + "/" + filename;
			var new_path = path.resolve("./uploads/") + "/" + filename + "." + filetype;
			// var now_path = config.BASEURL + filename;
			// var new_path = config.BASEURL + filename + "." + filetype;
			console.log(now_path, new_path)
...
exports.appupload = (req, res, next) => {
	if (req.files && req.files.length) {
		...
		var apkname = req.body.apkname;

		var new_path = config.BASEURL + apkname;

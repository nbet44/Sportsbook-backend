...
exports.getBalance = async (req, res, next) => {
    ...
    var history = await bwinHistoryModel.distinct("betId", { userId: data.userId, created: date })
    for (var i in history) {
        var betHistory = await baseController.BfindOne(bwinHistoryModel, { userId: data.userId, created: date, betId: history[i] })
        if (betHistory.status === "pending") {
            result.risk = result.risk + parseInt(betHistory.amount)
        } else if (betHistory.status === "win") {
            result.balance = result.balance - parseInt(betHistory.amount)
        } else {
            result.balance = result.balance - parseInt(betHistory.amount)
        }
    }
